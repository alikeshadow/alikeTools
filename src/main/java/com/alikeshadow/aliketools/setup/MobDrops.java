...
public class MobDrops {
    ...
    public void onMobDrops(LivingDropsEvent event) {
        ...
        if (event.getEntity() instanceof SquidEntity && Config.CALAMARI_DROP.get()) {
            ...
            ItemEntity drop = new ItemEntity(event.getEntity().world, event.getEntity().getPosX(), event.getEntity().getPosY(), event.getEntity().getPosZ(), stack);
        ...
        if (event.getEntity() instanceof ZombifiedPiglinEntity && Config.ZPIGMEN_DROP_PORKCHOPS.get()) {
