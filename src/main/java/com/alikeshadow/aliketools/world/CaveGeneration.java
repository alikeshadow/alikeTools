        ...
        for(Biome biome : ForgeRegistries.BIOMES){
            if (biome.getCategory() == Biome.Category.NETHER) {
                ...
                if (Config.STALAGMITE_GENERATION.get()) {
                    biome.addFeature(GenerationStage.Decoration.UNDERGROUND_DECORATION, Biome.createDecoratedFeature(
                            Feature.SIMPLE_BLOCK, new BlockWithContextConfig(
                                    ModBlocks.STALAGMITE.getDefaultState().with(BlockStateProperties.WATERLOGGED, Boolean.TRUE), placeOnSG, placeInW, placeUnderSG),
                            Placement.COUNT_RANGE,
                            new CountRangeConfig(Config.STALAGMITE_PER_CHUNK.get(), 5, 15, 90) //per chunk, height from layer 0, height from top layer, max spawn height
                ...
                if (Config.STALACTITE_GENERATION.get()) {
                    biome.addFeature(GenerationStage.Decoration.UNDERGROUND_DECORATION, Biome.createDecoratedFeature(
                            Feature.SIMPLE_BLOCK, new BlockWithContextConfig(
                                    ModBlocks.STALACTITE.getDefaultState().with(BlockStateProperties.WATERLOGGED, Boolean.TRUE), placeOnSC, placeInW, placeUnderSC),
                            Placement.COUNT_RANGE,
                            new CountRangeConfig(Config.STALACTITE_PER_CHUNK.get(), 5, 15, 90) //per chunk, height from layer 0, height from top layer, max spawn height
                    ));
                }
            }
            else {
                if (Config.STALAGMITE_GENERATION.get()) {
                    biome.addFeature(GenerationStage.Decoration.UNDERGROUND_DECORATION, Biome.createDecoratedFeature(
                            Feature.SIMPLE_BLOCK, new BlockWithContextConfig(
                                    ModBlocks.STALAGMITE.getDefaultState(), placeOnSG, placeIn, placeUnderSG),
                            Placement.COUNT_RANGE,
                            new CountRangeConfig(Config.STALAGMITE_PER_CHUNK.get(), 5, 15, 90) //per chunk, height from layer 0, height from top layer, max spawn height
                ...
                if (Config.STALACTITE_GENERATION.get()) {
                    biome.addFeature(GenerationStage.Decoration.UNDERGROUND_DECORATION, Biome.createDecoratedFeature(
                            Feature.SIMPLE_BLOCK, new BlockWithContextConfig(
                                    ModBlocks.STALACTITE.getDefaultState(), placeOnSC, placeIn, placeUnderSC),
                            Placement.COUNT_RANGE,
                            new CountRangeConfig(Config.STALACTITE_PER_CHUNK.get(), 5, 15, 90) //per chunk, height from layer 0, height from top layer, max spawn height
                    ));
                }
